...
def to_excel_percent(value):
    ...
    if isinstance(value, str):
        ...
        try:
            ...
            return value
        # If the original looked like a percent or the numeric value is > 1,
        # store it as Excel decimal percentage.
        if num > 1:
            return num / 100.0
        return num
...
def generate_from_mermas(mermas_path, tpl_art_path, tpl_chk_path, out_path, original_filename):
    ...
    for r in range(2, ws_top.max_row + 1):
        ...
        mc_map[key] = {'mapped': mapped, 'fam': fam, 'row': r, 'col_k': col_k}
    ...
    for key in mc_map:
        ...
        # H48: porcentaje real (normalizado a decimal Excel) y con formato %
        raw_h48 = mc_map[key]['mapped'][7] if len(mc_map[key]['mapped']) > 7 else None
        cell_h48 = new_mc.cell(row=48, column=8)
        cell_h48.value = to_excel_percent(raw_h48)

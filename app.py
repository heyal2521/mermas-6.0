...
def copy_sheet_exact(src_ws, tgt_wb, title):
    ...
    # Copiar valores y estilos respetando celdas fusionadas.
    # Las celdas secundarias de un merge son MergedCell y no admiten escritura.
    merged_cells = {}
    for merged in src_ws.merged_cells.ranges:
        min_col, min_row, max_col, max_row = merged.bounds
        for rr in range(min_row, max_row + 1):
            for cc in range(min_col, max_col + 1):
                merged_cells[(rr, cc)] = (rr == min_row and cc == min_col)

    for r in range(1, src_ws.max_row + 1):
        for c in range(1, src_ws.max_column + 1):
            s = src_ws.cell(row=r, column=c)
            if (r, c) in merged_cells and not merged_cells[(r, c)]:
                continue
            t = tgt.cell(row=r, column=c)
            try:
...
def generate():
    ...
    tpl_chk_new = os.path.join(os.path.dirname(__file__), 'CHECKLIST CALIDAD DE REPARTO(1).xlsx')
    tpl_chk_old = os.path.join(os.path.dirname(__file__), 'CHECKLIST CALIDAD DE REPARTO.xlsx')
    tpl_chk = tpl_chk_new if os.path.exists(tpl_chk_new) else tpl_chk_old
...
def generate_from_mermas(mermas_path, tpl_art_path, tpl_chk_path, out_path, original_filename):
    ...
    for key in mc_map:
        ...
        # E1 se mantiene vacío según la nueva plantilla
        chk.cell(row=1, column=5).value = None

        # D4:D10 -> exactamente el mismo contenido/formato que D3.
        for r in range(4, 11):
        ...
        # D11 -> comentarios (el resto de la revisión empieza en la nueva fila 12)
        chk.cell(row=11, column=4).value = "COMENTARIOS"

        # F3:F10 -> composición fija, recuadrada y centrada
        ...
        # Campos de revisión que ya no pertenecen a la nueva zona de composición.

...
def normalize_percent_value(value):
    """Normalize percentage-like values to Excel decimal form (e.g. 24 -> 0.24)."""
    if value is None:
        return None
...
def generate_from_mermas(mermas_path, tpl_art_path, tpl_chk_path, out_path, original_filename):
    ...
    for key in mc_map:
        ...
        # F48 se convierte a porcentaje visual/numérico
        cell = new_mc.cell(row=48, column=6)
        convert_pct_cell_to_number(cell)

        # H48 debe conservar un porcentaje real (normalizado a decimal Excel)
        cell_h48 = new_mc.cell(row=48, column=8)
        cell_h48.value = normalize_percent_value(cell_h48.value)

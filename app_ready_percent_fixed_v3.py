...
def generate_from_mermas(mermas_path, tpl_art_path, tpl_chk_path, out_path, original_filename):
    ...
    for key in mc_map:
        ...
        # H48: porcentaje real preservando el formato/valor de la celda de origen
        cell_h48 = new_mc.cell(row=48, column=8)
        raw_h48 = mc_map[key].get('h48_value')
        fmt_h48 = mc_map[key].get('h48_format')
        cell_h48.value = to_excel_percent_from_cell(raw_h48, fmt_h48)
        if fmt_h48 and '%' in str(fmt_h48):
            cell_h48.number_format = fmt_h48
        else:
